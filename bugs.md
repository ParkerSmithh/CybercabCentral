...
| Mobile header | The "Link Tesla Account" button is hidden below the `sm` breakpoint (`hidden sm:inline-flex`) with no alternate entry point, so it's unreachable on very small screens. |
